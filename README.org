...
To implement your own distribution, create a =class= or =struct= that implements two methods:
#+BEGIN_SRC c++
...
#+END_SRC
* Quantization
The *semidiscrete* transport problem can be used to quantize measures. A measure
quantization is a choice of points

#+BEGIN_SRC latex :exports none :file quantiles.png
\begin{equation*}
  x_1, \ldots, x_n
\end{equation*}
#+END_SRC
that approximate the original measure $\mu$ under some metric. The metric we
consider is the 2-Wasserstein distance between $\mu$ and the discrete
distribution supported on the $x_i$:
#+BEGIN_SRC latex :exports none :file quantization.png
\min_{x_1, \ldots, x_n} W_2^2\left(\mu, \sum_{i=1}^n \delta_{x_i} \right)
#+END_SRC
